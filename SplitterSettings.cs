...
namespace LiveSplit.BladedFury {
	public partial class SplitterSettings : UserControl {
		...
		private void Settings_Load(object sender, EventArgs e) {
			FindForm().Text = "Bladed Fury Autosplitter v" + Assembly.GetExecutingAssembly().GetName().Version.ToString(3);
			LoadSettings();
		}
		...
		private void btnDefaultSplits_Click(object sender, EventArgs e) {
			...
			Splits.Clear();
			Splits.Add(SplitName.StartGame);
			Splits.Add(SplitName.DukeKang);
			Splits.Add(SplitName.GeneralWu1);
			Splits.Add(SplitName.GeneralWu2);
			Splits.Add(SplitName.XiaZhengshu1);
			Splits.Add(SplitName.Wen);
